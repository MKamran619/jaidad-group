...
export function Footer() {
  ...
  return (
    <footer className="bg-[var(--surface)] text-[var(--text)]">
      {/* Newsletter Banner */}
      <div className="border-b border-[var(--border)]">
        <div className="container mx-auto px-4 py-10">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              <h3 className="font-display text-2xl font-bold text-[var(--text)] mb-1">Stay Updated</h3>
              <p className="text-[var(--text-muted)] text-sm">Get the latest property listings & news delivered to you</p>
            </div>
            <form className="flex gap-3 w-full md:w-auto" onSubmit={(e) => e.preventDefault()}>
              <Input
                placeholder="Your email address"
                type="email"
                className="md:w-72 bg-[var(--background)] border-[var(--border)] text-[var(--text)] placeholder:text-[var(--text-muted)] focus:ring-[var(--primary)]"
              />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          <div className="space-y-5">
            <Link to="/" className="flex items-center gap-3">
              ...
              <div>
                <p className="font-display font-bold text-[var(--text)] text-sm">Jaidad Group</p>
                <p className="text-[10px] text-[var(--text-muted)]">Premium Real Estate</p>
              </div>
            </Link>
            {footerDesc && (
              <p className="text-[var(--text-muted)] text-sm leading-relaxed">{footerDesc}</p>
            )}
              ...
              <div className="flex items-center gap-3">
                {socialLinks.map(({ icon: Icon, href, label }) => (
                  <a
                    ...
                    rel="noreferrer"
                    aria-label={label}
                    className="h-9 w-9 rounded-lg bg-[var(--surface-alt)] flex items-center justify-center text-[var(--text-muted)] hover:bg-[var(--primary)] hover:text-white transition-all duration-200"
                  >
          ...
          {/* Quick Links */}
          <div>
            <h4 className="font-semibold text-[var(--text)] mb-5 text-sm uppercase tracking-wider">Quick Links</h4>
            <ul className="space-y-3">
              {QUICK_LINKS.map((item) => (
                <li key={item.href}>
                  <Link
                    to={item.href}
                    className="text-[var(--text-muted)] text-sm hover:text-[var(--primary)] transition-colors flex items-center gap-2 group"
                  >
          ...
          {/* Properties */}
          <div>
            <h4 className="font-semibold text-[var(--text)] mb-5 text-sm uppercase tracking-wider">Properties</h4>
            <ul className="space-y-3">
          ...
          {/* Contact */}
          <div>
            <h4 className="font-semibold text-[var(--text)] mb-5 text-sm uppercase tracking-wider">Contact Us</h4>
            <ul className="space-y-4">
              {phone && (
                <li>
                  <a href={`tel:${phone}`} className="flex gap-3 text-sm text-[var(--text-muted)] hover:text-[var(--text)] transition-colors">
              ...
              {email && (
                <li>
                  <a href={`mailto:${email}`} className="flex gap-3 text-sm text-[var(--text-muted)] hover:text-[var(--text)] transition-colors">
      ...
      <div className="border-t border-[var(--border)]">
        <div className="container mx-auto px-4 py-5 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-[var(--text-muted)]">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link to="/sitemap" className="hover:text-[var(--text)] transition-colors">Sitemap</Link>
          </div>
        </div>
      </div>
    </footer>
