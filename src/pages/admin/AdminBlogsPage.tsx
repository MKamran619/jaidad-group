  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold text-[var(--text)]">Blog Posts</h1>
          <p className="text-sm text-[var(--text-muted)] mt-1">{blogs.length} articles</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="border-b border-[var(--border)] bg-[var(--background)]">
              <tr>
                <th className="text-left px-4 py-3 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Post</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Category</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Status</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Views</th>
            ...
            <tbody className="divide-y divide-[var(--border)]">
              {loading ? (
                ...
                <tr><td colSpan={6} className="text-center py-12 text-[var(--text-muted)]">No blog posts found</td></tr>
              ) : filtered.map((b) => (
                <tr key={b.id} className="hover:bg-[var(--background)] transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-[var(--text)] line-clamp-1">{b.title}</p>
                        <p className="text-xs text-[var(--text-muted)] line-clamp-1">{b.excerpt}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 text-[var(--text-muted)]">{b.view_count.toLocaleString()}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setShowForm(false)}>
          <div className="bg-[var(--surface)] rounded-2xl w-full max-w-2xl max-h-[92vh] overflow-y-auto shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="p-5 border-b border-[var(--border)] flex items-center justify-between sticky top-0 bg-[var(--surface)] z-10">
              <h2 className="font-display text-lg font-bold text-[var(--text)]">{editItem ? 'Edit' : 'New'} Blog Post</h2>
            ...
            <div className="flex border-b border-[var(--border)] px-5">
              {(['basic', 'content'] as const).map((tab) => (
                <button
                  ...
                  className={`py-3 px-4 text-sm font-medium border-b-2 transition-colors -mb-px capitalize ${activeTab === tab ? 'border-[var(--primary)] text-[var(--primary)]' : 'border-transparent text-[var(--text-muted)]'}`}
                >
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select value={form.category} onChange={(e) => f('category', e.target.value)} className="w-full h-10 px-3 rounded-xl border border-[var(--border)] bg-[var(--background)] text-[var(--text)] text-sm focus:outline-none focus:ring-2 focus:ring-[var(--primary)]">
                        {BLOG_CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-[var(--text-muted)] mb-1.5">Status</label>
                      <select value={form.status} onChange={(e) => f('status', e.target.value)} className="w-full h-10 px-3 rounded-xl border border-[var(--border)] bg-[var(--background)] text-[var(--text)] text-sm focus:outline-none focus:ring-2 focus:ring-[var(--primary)]">
                        <option value="draft">Draft</option>
                        <option value="published">Published</option>
                        <option value="archived">Archived</option>
                      </select>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="text-sm text-[var(--text)]">Featured Post</span>
                  </label>
                </>
              ) : (
                <>
                  <p className="text-xs text-[var(--text-muted)]">Write your blog content as HTML. You can use &lt;h2&gt;, &lt;p&gt;, &lt;ul&gt;, &lt;strong&gt;, etc.</p>
                  <Textarea
                    label="Content (HTML)"
                    value={form.content}
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setDeleteId(null)}>
          <div className="bg-[var(--surface)] rounded-2xl p-6 w-full max-w-sm shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-display text-lg font-bold text-[var(--text)] mb-2">Delete Blog Post?</h3>
