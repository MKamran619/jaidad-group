  ...
  return (
    <div className="flex h-screen bg-[var(--color-background)] overflow-hidden">
      ...
      <AnimatePresence initial={false}>
        {sidebarOpen && (
          <motion.aside
            ...
            className="flex-shrink-0 h-full border-r border-[var(--color-border)] bg-[var(--color-surface)] flex flex-col overflow-hidden"
          >
            {/* Logo */}
            <div className="flex h-16 items-center gap-3 px-5 border-b border-[var(--color-border)] flex-shrink-0">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[var(--color-brand-gold)] flex-shrink-0">
                <span className="font-display font-black text-white text-sm">J+</span>
              </div>
              <div className="min-w-0">
                <p className="font-display font-bold text-[var(--color-text)] text-sm truncate">Jaidad Group</p>
            ...
            <div className="border-t border-[var(--color-border)] p-3">
              <Link to="/" target="_blank" className="flex items-center gap-2 px-3 py-2 text-xs text-[var(--color-text-muted)] hover:text-[var(--color-text)] rounded-lg hover:bg-[var(--color-background)] transition-colors">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Top Bar */}
        <header className="h-16 flex-shrink-0 border-b border-[var(--color-border)] bg-[var(--color-surface)] flex items-center justify-between px-5">
          <div className="flex items-center gap-3">
            <button
              onClick={() => setSidebarOpen(!sidebarOpen)}
              className="h-9 w-9 rounded-xl border border-[var(--color-border)] flex items-center justify-center text-[var(--color-text-muted)] hover:bg-[var(--color-background)] transition-colors"
            >
          ...
          <div className="flex items-center gap-2">
            <button className="relative h-9 w-9 rounded-xl border border-[var(--color-border)] flex items-center justify-center text-[var(--color-text-muted)] hover:bg-[var(--color-background)] transition-colors">
              <FiBell className="h-4 w-4" />
              <span className="absolute top-1.5 right-1.5 h-2 w-2 rounded-full bg-[var(--color-brand-gold)]" />
            </button>
            <div className="h-9 w-9 rounded-xl bg-[var(--color-brand-gold)] flex items-center justify-center text-white font-bold text-xs">
              JG
            </div>
          </div>
        </header>
